...
<body>
  ...
  <script>
    const state = { key: sessionStorage.stampoAdminKey || '', tab: 'sights', rows: [], meta: { countries: [], collectionKinds: [] }, states: {}, cities: {}, filters: { sights: '', 'collection-lists': '' }, edit: null };
    ...
    const schemas = {
      ...
      collections: [['title', 'Title', 'text', 1], ['id', 'ID (optional)', 'text'], ['imageUrl', 'Image', 'image'], ['detail', 'Detail', 'textarea', 1], ['access', 'Access', 'access'], ['isPublished', 'Published', 'check']],
      'collection-lists': [['collectionKindId', 'Collection kind', 'kind', 1], ['title', 'Title', 'text', 1], ['id', 'ID (optional)', 'text'], ['countryId', 'Country', 'country', 1], ['state', 'State / region', 'state', 1], ['cityId', 'City / location', 'city', 1], ['imageUrl', 'Image', 'image'], ['detail', 'Detail', 'textarea', 1]],
    ...
    async function normalizeImage(input) { const file = input.files?.[0]; if (!file) return; try { const bitmap = await createImageBitmap(file); const canvas = document.createElement('canvas'); canvas.width = 1200; canvas.height = 800; const context = canvas.getContext('2d'); context.fillStyle = '#061f18'; context.fillRect(0, 0, 1200, 800); const scale = Math.min(1200 / bitmap.width, 800 / bitmap.height), width = bitmap.width * scale, height = bitmap.height * scale; context.drawImage(bitmap, (1200 - width) / 2, (800 - height) / 2, width, height); bitmap.close(); const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/jpeg', .9)); if (!blob) throw new Error('The image could not be resized.'); normalizedFiles.set(input, new File([blob], `${crypto.randomUUID()}.jpg`, { type: 'image/jpeg' })); note('Image resized to 1200 × 800 pixels.') } catch (error) { input.value = ''; note(error.message || 'The image could not be resized.', true) } }
    async function uploadImage(el) { const file = normalizedFiles.get(el) || el.files?.[0]; if (!file) return el.dataset.current || ''; const folders = { sights: 'sights', collections: 'collection', 'collection-lists': 'collection', 'daily-destinations': 'daily-destinations' }, body = new FormData(); body.append('image', file); body.append('folder', folders[form.dataset.resource]); const r = await fetch('/admin/api/images', { method: 'POST', headers: { Accept: 'application/json', Authorization: `Bearer ${state.key}`, 'X-Admin-Key': state.key }, body }); const type = r.headers.get('content-type') || ''; if (!type.includes('application/json')) throw new Error(`Image upload returned an invalid server response (${r.status}).`); const result = await r.json(); if (!r.ok) { const validation = Object.values(result.errors || {}).flat().join(' '); throw new Error(validation || result.message || 'Image upload failed.') } return result.imageUrl }
