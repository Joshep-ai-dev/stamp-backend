...
<body>
  ...
  <div id="app" class="shell hidden">
    <aside class="side">
      ...
      <nav class="nav"><button data-tab="countries" class="active">Country</button><button onclick="withButtonLoading(this, &quot;Loading...&quot;, () => showUsStates(this))">US state images</button><button data-tab="cities">Cities</button><button data-tab="sights">Top sights</button><button
          data-tab="collections">Collection kinds</button><button data-tab="collection-lists">Collection list</button><button data-tab="daily-destinations">Kroo IQ</button><button class="logout" onclick="logout()">Lock</button></nav>
    </aside>
    <main>
      <div class="top">
  ...
  <script>
    ...
    const schemas = {
      ...
      'daily-destinations': [['name', 'Kroo IQ lesson name', 'text', 1], ['id', 'Question ID (optional)', 'text'], ['countryId', 'Destination country', 'country', 1], ['state', 'State / region', 'state', 1], ['cityId', 'Destination city', 'city', 1], ['imageUrl', 'Lesson image', 'image'], ['icon', 'Fallback emoji', 'text'], ['content', 'Destination introduction and answer explanation', 'textarea', 1], ['question', 'Quiz question', 'textarea', 1], ['options', 'Answer choices (one per line)', 'textarea'], ['correctAnswer', 'Correct choice number (first choice is 0)', 'number', 1], ['publishDate', 'Quiz date (blank = reusable every day)', 'date'], ['isPublished', 'Available in Kroo IQ', 'check']]
    };
